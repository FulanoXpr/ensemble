...
import { appendMessage, getMessages } from './ensemble-registry.js'
import { getRuntime } from './runtime-singleton.js'
import { resolveAgentProgram } from './agent-config.js'
import { collabDeliveryFile } from './collab-paths.js'
import { isSelf } from './hosts-config.js'
...
// Stub: remote session commands are not supported in the VSCode extension (local only).
...
export class StagedWorkflowManager {
  ...
  private async deliverToAgent(agent: ActiveAgent, text: string): Promise<void> {
    ...
    // In VSCode extension v1, all agents are local.
    // The isSelf check is kept for structural compatibility but remote path is a no-op.
    if (agent.hostId && !isSelf(agent.hostId)) {
      await postRemoteSessionCommand('', sessionName, text)
      return
    }
  ...
  /**
   * Fetch messages incrementally using a cursor.
