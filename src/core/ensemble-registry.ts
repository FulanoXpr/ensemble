...
import type { EnsembleTeam, EnsembleMessage, CreateTeamRequest } from './types.js'
import { getEnsembleRegistryDir } from './ensemble-paths.js'
import { collabMessagesFile } from './collab-paths.js'

// CRITICAL: No module-level calls to getEnsembleRegistryDir().
// The extension context (and therefore storage dir) isn't available until activate() runs.
// All path resolution is done lazily via these getter functions.

function getDir(): string {
  return getEnsembleRegistryDir()
}

function getTeamsFile(): string {
  return path.join(getDir(), 'teams.json')
}

function getMessagesDir(): string {
  return path.join(getDir(), 'messages')
}

function getTeamsLockDir(): string {
  return `${getTeamsFile()}.lock`
}
...
export function appendMessage(teamId: string, message: EnsembleMessage): void {
  ...
  const dir = path.join(messagesDir, teamId)
  ensureDir(dir)
...
export function getMessages(teamId: string, since?: string): EnsembleMessage[] {
  const messagesDir = getMessagesDir()
  const sources = [
    path.join(messagesDir, teamId, 'feed.jsonl'),
